...
try {
	//Считаем ответ
	$flsWinner = fopen('C:/Users/Кристина/Downloads/OSPanel/domains/php-web/L1/tests/008.ans', 'rt');
	$win = explode(" ",  trim(fgets($flsWinner)));
	$win = intval($win[0]);
	fclose($flsWinner);
	$fp = fopen('C:/Users/Кристина/Downloads/OSPanel/domains/php-web/L1/tests/008.dat', 'rt');
	if(!$fp)
		throw new Exception ("Файл не найден.");
	//Считываем количество ставок и ставки
	...
	for ($i = 0; $i < $n; $i++) {
		list($a[], $s[], $com[]) = explode(" ", trim(fgets($fp)));
		$a[$i] = intval($a[$i]);
		$s[$i] = intval($s[$i]);
		$temp +=$s[$i];
		...
		if(
			(strcasecmp($com[$i], "L") != 0) && 
			(strcasecmp($com[$i], "R") != 0) && 
			(strcasecmp($com[$i], "D") != 0)
		)
		{
	...
	foreach ($a as $val) {
		...
		switch ($winner[$i]) {
			case "L":
				if (strcasecmp($winner[$i], $com[$inkrement]) == 0){
